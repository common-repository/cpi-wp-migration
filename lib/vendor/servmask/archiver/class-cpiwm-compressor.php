...
class Cpiwm_Compressor extends Cpiwm_Archiver {
	 ...
	 *
	 * @throws \Cpiwm_Not_Seekable_Exception
			...
			if ( ( $block = $this->get_file_block( $file_name, $new_file_name ) ) ) {
				...
				if ( $file_offset === 0 ) {
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new Cpiwm_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write header to file. File: %s', $this->file_name ) );
						}
					} else {
						throw new Cpiwm_Not_Writable_Exception( sprintf( 'Unable to write header to file. File: %s', $this->file_name ) );
					}
				}
						...
						if ( ( $file_content = @fread( $file_handle, 512000 ) ) !== false ) {
							if ( ( $file_bytes = @fwrite( $this->file_handle, $file_content ) ) !== false ) {
								if ( strlen( $file_content ) !== $file_bytes ) {
									throw new Cpiwm_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write content to file. File: %s', $this->file_name ) );
								}
							} else {
								throw new Cpiwm_Not_Writable_Exception( sprintf( 'Unable to write content to file. File: %s', $this->file_name ) );
							}
				...
				if ( ( $block = $this->get_file_size_block( $file_offset ) ) ) {
					...
					if ( @fseek( $this->file_handle, - $file_offset - 4096 - 12 - 14, SEEK_CUR ) === -1 ) {
						throw new Cpiwm_Not_Seekable_Exception(
					...
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new Cpiwm_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write size to file. File: %s', $this->file_name ) );
						}
					} else {
						throw new Cpiwm_Not_Writable_Exception( sprintf( 'Unable to write size to file. File: %s', $this->file_name ) );
					}
	...
	private function get_file_block( $file_name, $new_file_name = '' ) {
		...
		if ( ( $stat = @stat( $file_name ) ) !== false ) {
			...
			if ( empty( $new_file_name ) ) {
				$name = cpiwm_basename( $file_name );
			} else {
				$name = cpiwm_basename( $new_file_name );
			}
			...
			if ( empty( $new_file_name ) ) {
				$path = $this->replace_directory_separator_with_forward_slash( cpiwm_dirname( $file_name ) );
			} else {
				$path = $this->replace_directory_separator_with_forward_slash( cpiwm_dirname( $new_file_name ) );
			}
