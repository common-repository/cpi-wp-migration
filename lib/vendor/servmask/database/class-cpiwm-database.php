...
abstract class Cpiwm_Database {
	...
	public function __construct( $wpdb ) {
		...
		if ( is_resource( $this->wpdb->dbh ) ) {
			if ( get_resource_type( $this->wpdb->dbh ) === 'SQL Server Connection' ) {
				throw new Cpiwm_Database_Exception(
					'Your WordPress installation uses Microsoft SQL Server. ' .
					'To use CPI WP Migration, please change your installation to MySQL and try again. ' .
	...
	public function export( $file_name, &$table_index = 0, &$table_offset = 0, &$table_rows = 0 ) {
		// Set file handler
		$file_handler = cpiwm_open( $file_name, 'ab' );

		// Write headers
		if ( $table_index === 0 ) {
			cpiwm_write( $file_handler, $this->get_header() );
		}
		...
		for ( ; $table_index < count( $tables ); ) {
			...
			if ( in_array( $table_name, $views ) ) {
				...
				if ( $table_offset === 0 ) {
					...
					cpiwm_write( $file_handler, $drop_view );
					...
					cpiwm_write( $file_handler, $create_view );

					// Write end of statement
					cpiwm_write( $file_handler, ";\n\n" );
				}
				...
				if ( $table_offset === 0 ) {
					...
					cpiwm_write( $file_handler, $drop_table );
					...
					cpiwm_write( $file_handler, $create_table );
				...
				do {
					...
					if ( $primary_keys ) {
						...
						$query = sprintf( 'SELECT t1.* FROM `%s` AS t1 JOIN (SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d) AS t2 USING (%s)', $table_name, $table_keys, $table_name, $table_where, $table_keys, $table_offset, CPIWM_MAX_SELECT_RECORDS, $table_keys );
						...
						// Set query with offset and rows count
						$query = sprintf( 'SELECT * FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d', $table_name, $table_where, $table_keys, $table_offset, CPIWM_MAX_SELECT_RECORDS );
					}
					...
					if ( $num_rows = $this->num_rows( $result ) ) {
						...
						while ( $row = $this->fetch_assoc( $result ) ) {
							...
							if ( $table_offset % CPIWM_MAX_TRANSACTION_QUERIES === 0 ) {
								cpiwm_write( $file_handler, "START TRANSACTION;\n" );
							}
							...
							cpiwm_write( $file_handler, $table_insert );
							...
							if ( $table_offset % CPIWM_MAX_TRANSACTION_QUERIES === 0 ) {
								cpiwm_write( $file_handler, "COMMIT;\n" );
							}
						}
					} else {
					...
					// Time elapsed
					if ( ( $timeout = apply_filters( 'cpiwm_completed_timeout', 10 ) ) ) {
	...
	public function import( $file_name, &$query_offset = 0 ) {
		...
		// Set file handler
		$file_handler = cpiwm_open( $file_name, 'r' );
							...
							if ( $this->errno() === 1226 ) {
								if ( stripos( $this->error(), 'max_queries_per_hour' ) !== false ) {
									throw new Cpiwm_Database_Exception(
										...
										'To use CPI WP Migration, please increase MySQL max_queries_per_hour limit. ' .
										'<a href="#" target="_blank">Technical details</a>',
										503
									);
									...
									throw new Cpiwm_Database_Exception(
										...
										'To use CPI WP Migration, please increase MySQL max_updates_per_hour limit. ' .
										'<a href="#" target="_blank">Technical details</a>',
										503
									);
									...
									throw new Cpiwm_Database_Exception(
										...
										'To use CPI WP Migration, please increase MySQL max_connections_per_hour limit. ' .
										'<a href="#" target="_blank">Technical details</a>',
										503
									);
									...
									throw new Cpiwm_Database_Exception(
										...
										'To use CPI WP Migration, please increase MySQL max_user_connections limit. ' .
										'<a href="#" target="_blank">Technical details</a>',
										503
									);
								}
	...
	protected function replace_visual_composer_values_callback( $matches ) {
		...
		if ( Cpiwm_Database_Utility::base64_validate( $matches[1] ) ) {

			// Decode base64 characters
			$matches[1] = Cpiwm_Database_Utility::base64_decode( $matches[1] );

			// Replace values
			$matches[1] = Cpiwm_Database_Utility::replace_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[1] );

			// Encode base64 characters
			$matches[1] = Cpiwm_Database_Utility::base64_encode( $matches[1] );
		}
	...
	protected function replace_base64_values_callback( $matches ) {
		...
		if ( Cpiwm_Database_Utility::base64_validate( $matches[1] ) ) {
			...
			// Replace serialized values
			$matches[1] = Cpiwm_Database_Utility::replace_serialized_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[1] );
	...
	protected function replace_table_values_callback( $matches ) {
		...
		$matches[1] = Cpiwm_Database_Utility::unescape_mysql( $matches[1] );

		// Replace serialized values
		$matches[1] = Cpiwm_Database_Utility::replace_serialized_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[1] );

		// Escape MySQL special characters
		$matches[1] = Cpiwm_Database_Utility::escape_mysql( $matches[1] );
	...
	protected function replace_raw_values( $input ) {
		return Cpiwm_Database_Utility::replace_values( $this->get_old_replace_raw_values(), $this->get_new_replace_raw_values(), $input );
	}
