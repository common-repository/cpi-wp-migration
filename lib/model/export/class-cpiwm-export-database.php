...
class Cpiwm_Export_Database {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Cpiwm_Status::info( sprintf( __( 'Exporting database...<br />%d%% complete<br />%s records saved', CPIWM_PLUGIN_NAME ), $progress, number_format_i18n( $table_rows ) ) );
		...
		if ( isset( $params['options']['no_spam_comments'] ) ) {
			$mysql->set_table_where_clauses( cpiwm_table_prefix() . 'comments', array( "`comment_approved` != 'spam'" ) )
				->set_table_where_clauses( cpiwm_table_prefix() . 'commentmeta', array( sprintf( "`comment_ID` IN ( SELECT `comment_ID` FROM `%s` WHERE `comment_approved` != 'spam' )", cpiwm_table_prefix() . 'comments' ) ) );
		}

		// Exclude post revisions
		if ( isset( $params['options']['no_post_revisions'] ) ) {
			$mysql->set_table_where_clauses( cpiwm_table_prefix() . 'posts', array( "`post_type` != 'revision'" ) );
		}
		...
		// Set table prefixes
		if ( cpiwm_table_prefix() ) {
			$old_table_prefixes[] = $old_column_prefixes[] = cpiwm_table_prefix();
			$new_table_prefixes[] = $new_column_prefixes[] = cpiwm_servmask_prefix();
		} else {
			// Set table prefixes based on table name
			foreach ( $mysql->get_tables() as $table_name ) {
				$old_table_prefixes[] = $table_name;
				$new_table_prefixes[] = cpiwm_servmask_prefix() . $table_name;
			}

			// Set table prefixes based on column name
			foreach ( array( 'user_roles' ) as $option_name ) {
				$old_column_prefixes[] = $option_name;
				$new_column_prefixes[] = cpiwm_servmask_prefix() . $option_name;
			}
			...
			foreach ( array( 'capabilities', 'user_level', 'dashboard_quick_press_last_post_id', 'user-settings', 'user-settings-time' ) as $meta_key ) {
				...
				$new_column_prefixes[] = cpiwm_servmask_prefix() . $meta_key;
			}
		}
		...
		if ( cpiwm_table_prefix() ) {
			$include_table_prefixes[] = cpiwm_table_prefix();
		} else {
			foreach ( $mysql->get_tables() as $table_name ) {
		...
		$mysql->set_table_where_clauses( cpiwm_table_prefix() . 'options', array( sprintf( "`option_name` NOT IN ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')", CPIWM_ACTIVE_PLUGINS, CPIWM_ACTIVE_TEMPLATE, CPIWM_ACTIVE_STYLESHEET, CPIWM_STATUS, CPIWM_SECRET_KEY, CPIWM_AUTH_USER, CPIWM_AUTH_PASSWORD, CPIWM_BACKUPS_LABELS, CPIWM_SITES_LINKS ) ) );

		// Replace table prefix on columns
		$mysql->set_table_prefix_columns( cpiwm_table_prefix() . 'options', array( 'option_name' ) )
			->set_table_prefix_columns( cpiwm_table_prefix() . 'usermeta', array( 'meta_key' ) );

		// Export database
		if ( $mysql->export( cpiwm_database_path( $params ), $table_index, $table_offset, $table_rows ) ) {

			// Set progress
			Cpiwm_Status::info( __( 'Done exporting database.', CPIWM_PLUGIN_NAME ) );
