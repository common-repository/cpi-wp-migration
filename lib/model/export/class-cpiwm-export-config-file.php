...
class Cpiwm_Export_Config_File {
	...
	public static function execute( $params ) {
		...
		if ( isset( $params['archive_bytes_offset'] ) ) {
			...
			$archive_bytes_offset = cpiwm_archive_bytes( $params );
		}
		...
		if ( isset( $params['total_package_size'] ) ) {
			...
			$total_package_size = cpiwm_package_bytes( $params );
		}
		...
		// Set progress
		Cpiwm_Status::info( sprintf( __( 'Archiving configuration file...<br />%d%% complete', CPIWM_PLUGIN_NAME ), $progress ) );
		...
		if ( $archive->add_file( cpiwm_package_path( $params ), CPIWM_PACKAGE_NAME, $package_bytes_written, $package_bytes_offset ) ) {

			// Set progress
			Cpiwm_Status::info( __( 'Done archiving configuration file.', CPIWM_PLUGIN_NAME ) );
