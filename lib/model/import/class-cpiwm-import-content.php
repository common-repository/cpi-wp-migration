...
class Cpiwm_Import_Content {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Cpiwm_Status::info( sprintf( __( 'Restoring %d files...<br />%d%% complete', CPIWM_PLUGIN_NAME ), $total_files_count, $progress ) );
		...
		foreach ( $blogs as $blog ) {
			if ( cpiwm_main_site( $blog['Old']['BlogID'] ) === false ) {
				if ( defined( 'UPLOADBLOGSDIR' ) ) {
					// Old sites dir style
					$old_paths[] = cpiwm_files_path( $blog['Old']['BlogID'] );
					$new_paths[] = cpiwm_files_path( $blog['New']['BlogID'] );

					// New sites dir style
					$old_paths[] = cpiwm_sites_path( $blog['Old']['BlogID'] );
					$new_paths[] = cpiwm_files_path( $blog['New']['BlogID'] );
				} else {
					// Old sites dir style
					$old_paths[] = cpiwm_files_path( $blog['Old']['BlogID'] );
					$new_paths[] = cpiwm_sites_path( $blog['New']['BlogID'] );
		...
		while ( $archive->has_not_reached_eof() ) {
			...
			$exclude_files = array_merge(
				...
				array(
					...
					CPIWM_DATABASE_NAME,
					CPIWM_MUPLUGINS_NAME,
				)
			);

			// Exclude Elementor files
			$exclude_files = array_merge( $exclude_files, array( CPIWM_ELEMENTOR_CSS_NAME ) );

			// Exclude content extensions
			$exclude_extensions = array( CPIWM_LESS_CACHE_NAME );
