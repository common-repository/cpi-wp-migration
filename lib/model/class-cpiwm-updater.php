...
class Cpiwm_Updater {
	...
	public static function plugins_api( $result, $action = null, $args = null ) {
		...
		if ( isset( $args->slug ) && isset( $extensions[ $args->slug ] ) && $action === 'plugin_information' ) {
			...
			// Get current updates
			$updates = get_option( CPIWM_UPDATER, array() );
	...
	public static function check_for_updates() {
		...
		// Get extension updates
		foreach ( Cpiwm_Extensions::get() as $slug => $extension ) {
		...
		return update_option( CPIWM_UPDATER, $updates );
	}
		...
		foreach ( Cpiwm_Extensions::get() as $slug => $extension ) {
			...
			if ( $file === $extension['basename'] ) {
				...
				if ( get_option( $extension['key'] ) ) {
					...
					// Add "Check for updates" link
					$links[] = Cpiwm_Template::get_content(
						'updater/check',
						array(
							'url' => $updater_url,
						)
