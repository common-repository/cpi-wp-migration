...
jQuery(document).ready(function ($) {
	...
	$('#cpiwm-backups-list').on({
		mouseenter: function mouseenter() {
			$(this).addClass('cpiwm-button-on');
		},
		mouseleave: function mouseleave() {
			$(this).removeClass('cpiwm-button-on');
		}
	}, '.cpiwm-backup-actions > a');

	// Delete file
	$('#cpiwm-backups-list').on('click', '.cpiwm-backup-delete', function (e) {
		var self = $(this);

		// Delete file
		/* eslint-disable no-alert */
		if (confirm(cpiwm_locale.want_to_delete_this_file)) {
			/* eslint-enable no-alert */
			$.ajax({
				url: cpiwm_backups.ajax.url,
				type: 'POST',
				dataType: 'json',
				data: {
					secret_key: cpiwm_backups.secret_key,
				...
				if (data.errors.length === 0) {
					...
					if ($('.cpiwm-backups tbody tr').length === 1) {
						$('.cpiwm-backups').hide();
						$('.cpiwm-backups-empty').show();
					}
				}
			});
		}
	...
	// Restore from file
	$('#cpiwm-backups-list').on('click', '.cpiwm-backup-restore', function (e) {
		e.preventDefault();

		/* eslint-disable no-unused-vars */
		if (Cpiwm.MultisiteExtensionRestore) {
			var restore = new Cpiwm.MultisiteExtensionRestore($(this).data('archive'));
		} else if (Cpiwm.UnlimitedExtensionRestore) {
			var _restore = new Cpiwm.UnlimitedExtensionRestore($(this).data('archive'), $(this).data('size'));
		} else if (Cpiwm.FreeExtensionRestore) {
			var _restore2 = new Cpiwm.FreeExtensionRestore($(this).data('archive'));
		} else {
			var _restore3 = new Cpiwm.Restore($(this).data('archive'));
		}
		/* eslint-enable no-unused-vars */
	});

	$('#cpiwm-backups-list').on('click', '.cpiwm-backup-label-description, .cpiwm-backup-label-text', function () {
		$(this).hide();
		$(this).closest('.cpiwm-column-name').find('.cpiwm-backup-label-holder').show();
		$(this).closest('.cpiwm-column-name').find('.cpiwm-backup-label-field').focus();
	});

	$('#cpiwm-backups-list').on('keydown', '.cpiwm-backup-label-field', function (e) {
		var self = $(this);
		...
		if (e.which === 13) {
			e.preventDefault();
			self.hide();
			self.closest('.cpiwm-backup-label-holder').append(spinner);

			$.ajax({
				url: cpiwm_backups.labels.url,
				type: 'POST',
				dataType: 'json',
				data: {
					secret_key: cpiwm_backups.secret_key,
				...
				if (data.errors.length === 0) {
					...
					if (self.val()) {
						self.closest('.cpiwm-backup-label-holder').hide();
						self.closest('.cpiwm-column-name').find('.cpiwm-backup-label-text').show();
						self.closest('.cpiwm-column-name').find('.cpiwm-backup-label-colored').text(self.val());
					} else {
						self.closest('.cpiwm-backup-label-holder').hide();
						self.closest('.cpiwm-column-name').find('.cpiwm-backup-label-description').removeClass('cpiwm-backup-label-selected').removeAttr('style');
					}
			...
			if (self.data('value')) {
				...
				self.closest('.cpiwm-backup-label-holder').hide();
				self.closest('.cpiwm-column-name').find('.cpiwm-backup-label-text').hide();
				self.closest('.cpiwm-column-name').find('.cpiwm-backup-label-description').removeClass('cpiwm-backup-label-selected').removeAttr('style');
			}
	...
	$(document).on('cpiwm-export-status', function (e, params) {
		if (params.type === 'download') {
			if ($('.cpiwm-backups tbody tr').length > 1) {
				$('.cpiwm-backups-list-spinner-holder').show();
			} else {
				$('.cpiwm-backups-empty').hide();
				$('.cpiwm-backups-empty-spinner-holder').show();
			}

			$.get(cpiwm_backups.backups.url, { secret_key: cpiwm_backups.secret_key }).done(function (data) {
				$('#cpiwm-backups-create').find('.cpiwm-backups-empty').hide();
				$('#cpiwm-backups-create').find('.cpiwm-backups-empty-spinner-holder').hide();
				$('#cpiwm-backups-list').html(data);
			});
		}
	});
	...
	$('#cpiwm-create-backup').click(function (e) {
...
global.Cpiwm = jQuery.extend({}, global.Cpiwm, { Feedback: Feedback, Report: Report, Import: Import, Restore: Restore, Export: Export });
...
var Restore = function Restore() {
  var model = new Import();
  model.setStatus({ type: 'pro', message: cpiwm_locale.restore_from_file });
};
